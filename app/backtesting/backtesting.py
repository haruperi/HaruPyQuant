...
class Trade:
    ...
    def pl_pct(self):
        ...
        price = self.__exit_price or self.__broker.last_price
        # Ensure entry price is not zero to avoid division by zero
        if self.__entry_price == 0:
            return np.nan # Or some other appropriate value
        return copysign(1, self.__size) * (price / self.__entry_price - 1) * 100 # Multiply by 100 for percentage

    @property
    def pl_pips(self):
        """Trade profit (positive) or loss (negative) in pips."""
        price = self.__exit_price or self.__broker.last_price
        # Access pip size safely
        pip_size = getattr(getattr(self.__broker, '_data', None), 'pip', None)
        if not pip_size or pip_size == 0:
            return np.nan  # Cannot calculate pips without valid pip size
        # Pip calculation: (exit - entry) / pip_size, adjusted for direction
        return copysign(1, self.__size) * (price - self.__entry_price) / pip_size / 10
