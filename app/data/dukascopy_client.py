...
def _stream(
    ...
    while True:
        try:
            ...
            if not is_first_iteration and lastUpdates and lastUpdates[0] and lastUpdates[0][0] == cursor:
                lastUpdates = lastUpdates[1:]

            if not lastUpdates or not lastUpdates[0]:
                if end is not None:
                    break
                else:
                    continue
    ...
    for tick_count, row in enumerate(datafeed, 0):
        ...
        if time_unit == TIME_UNIT_TICK and interval_value == 1:
            if completed_bars_only:
                tick_df = pd.DataFrame([row[1:]], columns=df.columns, index=[timestamp])
        ...
        if (
            ...
            if open is not None:
                bar_data = [open, high, low, close, volume]
                if completed_bars_only:
                    bar_df = pd.DataFrame(
                        [bar_data], columns=df.columns, index=[last_timestamp]
                    )
                    bar_df.index.name = "timestamp"
                    yield bar_df
                else:
                    df.loc[last_timestamp] = bar_data
                    yield df
