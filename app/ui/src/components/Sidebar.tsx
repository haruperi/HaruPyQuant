...
const fundamentalsTools = [
  { name: 'Calendar', href: '/calendar', icon: <MdCalendarToday size={18} /> },
  { name: 'News', href: '/news', icon: <MdBook size={18} /> },
];
      ...
      <nav className="flex-1">
        <ul className="space-y-2">
          ...
          <li>
            <button
              className={`flex items-center w-full px-3 py-2 rounded transition-colors focus:outline-none ${
                fundamentalsTools.some(tool => pathname.startsWith(tool.href))
                  ? 'bg-blue-600 text-white shadow-lg'
                  : 'hover:bg-[#23272F] text-gray-300'
              }`}
            ...
            >
              <span className={`mr-3 ${fundamentalsTools.some(tool => pathname.startsWith(tool.href)) ? 'text-white' : 'text-blue-300'}`}>
                <MdAssessment size={22} />
              </span>
              <span className="flex-1 text-left">Fundamentals</span>
              <span>{fundamentalsOpen ? <MdExpandLess size={20} /> : <MdExpandMore size={20} />}</span>
            </button>
            {fundamentalsOpen && (
              <ul className="ml-8 mt-1 space-y-1">
                {fundamentalsTools.map((tool) => {
          ...
          <li>
            <button
              className={`flex items-center w-full px-3 py-2 rounded transition-colors focus:outline-none ${
                tradingTools.some(tool => pathname.startsWith(tool.href))
                  ? 'bg-blue-600 text-white shadow-lg'
                  : 'hover:bg-[#23272F] text-gray-300'
              }`}
            ...
            >
              <span className={`mr-3 ${tradingTools.some(tool => pathname.startsWith(tool.href)) ? 'text-white' : 'text-blue-300'}`}>
                <MdBuild size={22} />
              </span>
